...
const parse_baidu=(html="")=>{
    const $=cheerio.load(html)
    let window={}
    let css="script:not([src])"
    let s=$(css)
    var V={}
    for (let i=0;i<s.length;i++){
        const js=s.eq(i).html()
        if (/area/.test(js)){
            console.log(i)
            try{
                let js1=js.replace(/require.*/,'')
                eval(js1)
            }catch(e){
                console.log(e)
            }
...
const get_baidu=async ()=>{
    let h={
            ...HEADERS,
            "Host" : "voice.baidu.com",
    }
    let u="https://voice.baidu.com/act/newpneumonia/newpneumonia"
    let r=await superagent
        .get(u)
        .set(h)
    return parse_baidu(r.text)
}


const test=async ()=>{
    let u=require('utility')
    let r=await get_baidu()
